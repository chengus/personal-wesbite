...
4. The visitor bookmarks the page and uses **Check for replies** to reload it. They can post followups on the same page; each notification uses `In-Reply-To` and `References` to reply to the latest recorded email in that conversation. The subject remains the same with a `Re:` prefix. Enable Conversation View in Gmail to display the chain together.
...
For conversations started before the threading update, reply once to an existing notification from Gmail to connect future website followups to that old email chain. Earlier email IDs were not retained and cannot be reconstructed from the database. New sends save the provider’s Message-ID; authenticated incoming replies save their Message-ID and references. Email IDs stay out of the visitor page. If a binding returns no usable Message-ID, the next owner reply can establish the connection.
